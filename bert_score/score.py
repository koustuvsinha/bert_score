...
__all__ = ["score", "plot_example", "score_with_two_models"]
...
def score_with_two_models(
    ...
    assert model_type_1 is not None and model_type_2 is not None, "Specify both model types"
    ...
    #     model_type = lang2model[lang]
    if num_layers is None:
        num_layers = model2layers[model_type_1]

    tokenizer = get_tokenizer(model_type_1)
    model_1 = get_model(model_type_1, num_layers, all_layers)
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    model_1.to(device)
    model_2 = get_model(model_type_2, num_layers, all_layers)
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    model_2.to(device)
    ...
    if verbose:
        print("calculating scores...")
    start = time.perf_counter()
    all_preds = bert_cos_score_idf_two_model(
        model_1,
        model_2,
        refs,
        cands,
    ...
    if rescale_with_baseline:
        ...
        if os.path.isfile(baseline_path):
            ...
            print(
                f"Warning: Baseline not Found for {model_type_1} on {lang} at {baseline_path}", file=sys.stderr,
            )
    ...
    if return_hash:
        return tuple(
            [
                out,
                get_hash(model_type_1, num_layers, idf, rescale_with_baseline, use_custom_baseline=use_custom_baseline,),
            ]
        )
